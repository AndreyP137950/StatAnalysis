...
void generateSamples(vector<double>& sample1, vector<double>& sample2, double& alpha) {
    ...
    double s1 = 0.01;  // генеральное СКО первой совокупности
    ...
    double s2 = 0.01;  // генеральное СКО второй совокупности
...
void writeOutput(const string& fileName,
    ...
    fout << "Генеральная дисперсия sigma1^2 = " << (trueStd1 * trueStd1) << "\n";
    ...
    fout << "Генеральная дисперсия sigma2^2 = " << (trueStd2 * trueStd2) << "\n";
